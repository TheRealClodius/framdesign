...
const nextConfig: NextConfig = {
  ...
  webpack: (config, { isServer, dev }) => {
    ...
    if (isServer) {
      ...
      // Copy tool_registry.json to serverless function output directory
      // This ensures the file is available at runtime in Vercel deployments
      // The file is generated by 'npm run build:tools' before the Next.js build
      // CopyPlugin copies files relative to webpack's output, but we need it
      // in the serverless function's file system, so we rely on outputFileTracingIncludes
      // CopyPlugin here ensures it's available during build, but the actual inclusion
      // happens via outputFileTracingIncludes configuration above
        ...
        config.plugins.push(
          new CopyPlugin({
            patterns: [
              {
                from: registryPath,
                to: join(process.cwd(), '.next', 'server', 'tools', 'tool_registry.json'),
