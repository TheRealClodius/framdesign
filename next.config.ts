...
const nextConfig: NextConfig = {
  ...
  webpack: (config, { isServer, dev }) => {
    ...
    if (isServer) {
      ...
      // This is a workaround for outputFileTracingIncludes not working reliably
      // The file is generated by 'npm run build:tools' before the Next.js build
      // We copy it maintaining the directory structure so it's available at runtime
      const registryPath = join(process.cwd(), 'tools', 'tool_registry.json');
      if (existsSync(registryPath)) {
        // Use webpack's CopyPlugin to copy the file
        // Copy to .next/server/tools/ to maintain directory structure
        ...
        config.plugins.push(
          new CopyPlugin({
            patterns: [
              {
                from: registryPath,
                to: 'tools/tool_registry.json', // Relative to output directory
                context: process.cwd(),
